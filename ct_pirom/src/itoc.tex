...
\begin{frame}{\large \textbf{Step back from direct-shooting methods} and consider a \textbf{control-theoretic} training,}
    \footnotesize
    \only<2>{
        \begin{figure}
            \centering
...
\begin{frame}
    \centering
    \Large\textbf{Addressing the Impossible Trinity of Optimal Control (ITOC)}
\end{frame}

\begin{frame}{\large How can we leverage the \textbf{control-theoretic} training for optimal closed-loop control?}
    \visible<2->{
    \begin{center}
        {\Large \textbf{Consider the following example}:}\\
        \vspace*{0.2in}
        {On-board optimal closed-loop control of a hypersonic vehicle}
    \end{center}
    \vspace*{-0.2in}
    \begin{figure}
        \centering
...
\begin{frame}{\large We want \textbf{accuracy, efficient, and generalizable} solutions to \textbf{non-linear OCPs},}
    \footnotesize
    \begin{columns}
...
\begin{frame}{\large Generate a sparse high fidelity \textbf{distribution of optimal paths},}
    \footnotesize
    \only<1>{
        \begin{figure}
            \centering
...
\begin{frame}{\large \textbf{Learn the value function} from the sparse high fidelity data,}
    \footnotesize
    \only<2>{
        \begin{figure}
            \centering
...
\begin{frame}{\large We can \textbf{learn the value function} with controllable \textbf{efficiency and accuracy},}
    \footnotesize
    \only<2>{
        \begin{figure}
            \centering
...
\begin{frame}{\large Real-time control for the \textbf{space shuttle during re-entry} to maximize \textbf{cross-range},}
    \footnotesize
    \begin{columns}
        \begin{column}{0.7\textwidth}
            \only<2>{
...
\begin{frame}{\large Is the Impossible Trinity of Optimal Control (ITOC) \textbf{solved}?}
    \footnotesize
    \begin{columns}
        \begin{column}{0.4\textwidth}
            \centering
    ...
    \begin{tikzpicture}[remember picture, overlay]
        \node[anchor=north west, xshift=2.2cm, yshift=-2.2cm] at (current page.north west) {\includegraphics[width=0.1\textwidth]{../figs/pirom/checkmark.png}};
    \end{tikzpicture}}
    \visible<4->{
    \begin{tikzpicture}[remember picture, overlay]
        \node[anchor=north west, xshift=0.1cm, yshift=-5.1cm] at (current page.north west) {\includegraphics[width=0.1\textwidth]{../figs/pirom/checkmark.png}};
    \end{tikzpicture}}
    \visible<7->{
    \begin{tikzpicture}[remember picture, overlay]
        \node[anchor=north west, xshift=4.45cm, yshift=-5.1cm] at (current page.north west) {\includegraphics[width=0.1\textwidth]{../figs/pirom/checkmark.png}};
    \end{tikzpicture}}
\end{frame}
