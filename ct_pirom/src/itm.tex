...
% \begin{frame}{\small How does PIROM \textbf{solve the ITM} in small-data regimes?}
%     \footnotesize
%     \only<1>{
%     \begin{figure}
%         \centering
...
% \begin{frame}{\small Consider the following hypersonic TPS example,}
%     \footnotesize
%     \begin{columns}
%         \visible<2->{
...
\begin{frame}{\small Next, we employ the Mori-Zwanzig formalism to \textbf{account for the residual dynamics},}
    \footnotesize
    \visible<2->{
    \begin{equation*}
...
\begin{frame}{\small Next, perform \textbf{physics infusion} by \textbf{approximating} the memory kernel,}
    \footnotesize
    \begin{columns}
...
% \begin{frame}{\small Learn the hidden dynamics using direct shooting methods,}
%     \footnotesize
%     \begin{columns}
...
% \begin{frame}{\small Now, consider the following TPS \textbf{parametrization},}
%     \footnotesize
%     \only<2>{
%         \begin{figure}
%             \centering
...
\begin{frame}{\small Is the Impossible Trinity of Modeling \textbf{solved}?}
    ...
    \begin{columns}
        ...
        \begin{column}{0.8\textwidth}
            ...
            \begin{itemize}
                ...
                \item<4-> \colorbox{green!30}{\textbf{Generalizability}}:
                \begin{itemize}
                    \item<5-> \colorbox{green!30}{\textbf{Dimensions}: un-seen loading conditions},
                    \item<6-> \colorbox{green!30}{\textbf{Physics}: un-seen material properties} 
                \end{itemize}
                \visible<7->{
                \item \textbf{What about training efficiency?}}
                \begin{itemize}
                    \item<8-> \textbf{Overhead} is \colorbox{red!30}{$\approx$ 4 hrs in 3 CPUs} (similar to NODE),
                    \item<9-> therefore, we need to explore \colorbox{red!30}{other training approaches!}
                \end{itemize}
            \end{itemize}
        \end{column}
    \end{columns}
    ...
    \begin{tikzpicture}[remember picture, overlay]
        \node[anchor=north west, xshift=1.87cm, yshift=-3.2cm] at (current page.north west) {\includegraphics[width=0.05\textwidth]{../figs/pirom/checkmark.png}};
    \end{tikzpicture}}
    \visible<3->{
    \begin{tikzpicture}[remember picture, overlay]
        \node[anchor=north west, xshift=0.24cm, yshift=-5.4cm] at (current page.north west) {\includegraphics[width=0.05\textwidth]{../figs/pirom/checkmark.png}};
    \end{tikzpicture}}
    \visible<4->{
    \begin{tikzpicture}[remember picture, overlay]
        \node[anchor=north west, xshift=3.58cm, yshift=-5.4cm] at (current page.north west) {\includegraphics[width=0.05\textwidth]{../figs/pirom/checkmark.png}};
    \end{tikzpicture}}
\end{frame}